...
def get_structure_info_from_properties_json(
...
) -> tuple[libcasm.xtal.Lattice, np.ndarray, list[str]]:
    ...
    casm_lattice = libcasm.xtal.Lattice(np.transpose(lattice_row_vector_matrix))
    ...
    if properties_json["coordinate_mode"] == "Cartesian":
        frac_coords = libcasm.xtal.cartesian_to_fractional(casm_lattice, frac_coords)
...
def casm_structure_from_properties_json(
    properties_json: dict,
    ...
    return libcasm.xtal.Structure(casm_lattice, frac_coords, atom_types)


def casm_prim_from_properties_json(properties_json: dict) -> libcasm.xtal.Prim:
